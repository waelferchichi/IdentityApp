    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> ForgetUsernameOrPassword(string email)
        {
            ...
            try
            {
                ...
                {
                    return Ok(new JsonResult(new { title = "Forget username or password email sent", message = "Please check your email" }));
                }
        ...
        private async Task<bool> SendConfirmEMailAsync(User user)
        {
            ...
            var url = $"{_config["JWT:CleintUrl"]}/{_config["Email:ConfirmEmailPath"]}?token={token}&email={user.Email}";
        ...
        private async Task<bool> SendForgotUsernameOrPasswordEmail(User user)
        {
            ...
            token = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(token));
            var url = $"{_config["JWT:CleintUrl"]}/{_config["Email:ResetPasswordPath"]}?token={token}&email={user.Email}";
            ...
            var emailSend = new EmailSendDto(user.Email, "Forge", body);
